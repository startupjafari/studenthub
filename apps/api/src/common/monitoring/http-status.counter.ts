...
// Счётчик ответов по статусам.
//
// Отвечает на два вопроса наблюдения: «нет ли перебора паролей» (всплеск 401/403/429) и
// «какова доля 5xx» за окно. Оба нужны в агрегате, поэтому счётчик один: два похожих,
// пишущих минутные корзины бок о бок, разошлись бы в числах.
//
// Живёт в `common`, потому что видят все ответы только глобальные фильтр и интерцептор,
// а они здесь. Читатели счётчика в него ничего не пишут.
//
...
export class HttpStatusCounter {
  ...
  // читают заметно реже. Сброс происходит на первом ответе новой минуты: отдельный таймер
  // тикал бы и тогда, когда счётчик никто не читает.
