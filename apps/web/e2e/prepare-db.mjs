...
// Справочник КАТО заливается тем же прогоном (первый шаг сида): `seed.mjs` кладёт в
// `University.city` код, и без справочника город вуза не во что резолвить — экраны
// платформенного админа показали бы цифры вместо названий.
console.log('e2e: заполняю тестовыми данными (включая справочник КАТО)')
