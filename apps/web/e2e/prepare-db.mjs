...
// Справочник КАТО: `seed.mjs` кладёт в `University.city` код, и без справочника
// город вуза не во что резолвить — экраны платформенного админа показали бы цифры.
console.log('e2e: заливаю справочник КАТО')
run('node', ['prisma/seed-kato.mjs'])
