...
export function Table({
   ...
   * контента, а строки делят эту высоту между собой — на высоком экране страница из
   * двадцати строк заполняет карточку целиком, а не половину. Строк больше, чем места, —
   * каждая остаётся своей нормальной высоты, и тело таблицы прокручивается. Списки
   * короче десяти строк не растягиваются вовсе: остаются нормальной высоты.
  ...
  return (
    <table
      ...
      className={cn(
        ...
        // Тело — flex-колонка на всю свободную высоту, строки делят её поровну:
        // `grow` растягивает их, пока есть свободное место (страница из двадцати строк
        // заполняет карточку целиком), а `shrink-0` не даёт сжаться ниже своей высоты,
        // когда строк больше, чем места, — тогда включается прокрутка тела. Минимум для
        // строки — её собственная высота (padding ячейки), отдельным `min-h` не задаётся.
        //
        // Растягиваются только списки от десяти строк: `:has(> tr:nth-child(10))` — это и
        // есть проверка «строк не меньше десяти», считать их в JS не нужно. Три строки во
        // весь экран выглядят сломанной вёрсткой, а не заполненной таблицей, поэтому
        // короткий список остаётся нормальной высоты, со свободным местом под собой.
        scrollBody &&
          fill &&
          'flex min-h-0 flex-1 flex-col [&>tbody]:flex [&>tbody]:min-h-0 [&>tbody]:max-h-none [&>tbody]:flex-1 [&>tbody]:flex-col [&>tbody>tr]:shrink-0 [&>tbody:has(>tr:nth-child(10))>tr]:grow',
