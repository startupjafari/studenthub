...
/**
 * Текст сообщения простой строкой — для превью в панелях ответа и правки и в списках.
 *
 * В поле ввода лежит markdown, и в превью он уезжал как есть: человек, правящий
 * «**срочно** по `ТЗ`», видел над полем ровно эти звёздочки и обратные кавычки. Показывать
 * в однострочной справке разметку бессмысленно — форматирование там всё равно не читается,
 * а символы засоряют строку.
 *
 * Рендерить markdown по-настоящему тут нельзя: превью обязано остаться одной строкой с
 * обрезкой, а вложенные элементы её ломают. Поэтому разметка именно снимается.
 */
