...
Повторное использование инвалидированного токена рвёт всю цепочку — кроме **окна грации**
(`REFRESH_REUSE_GRACE_MS`, по умолчанию 10 с): повтор ТОЛЬКО ЧТО ротированного токена при живой
цепочке трактуется как недоставленный ответ, а не кража. Браузер обрывает запрос при навигации,
закрытии вкладки или выгрузке страницы на мобильном — сервер ротацию выполнил, а новая cookie до
клиента не доехала. В окне выдаётся новая ротация в той же цепочке (остаётся ровно один живой
токен); вне окна — прежний строгий разрыв. `0` отключает послабление.
...
### 8.1 Прямая (presigned) загрузка — три шага и три недоверия

`presign` → `PUT` в MinIO браузером → `confirm`. Реализация одна для всех модулей:
`FileService.presignPut` / `confirmDirectUpload`; модуль передаёт только бакет и свой белый
список MIME (документ — PDF/JPG/PNG, медиа профиля — фото/видео).

На шаге `confirm` API **не видел содержимого файла**, поэтому не верит запросу ни в чём:

1. **Ключ** обязан начинаться с id владельца (`presignPut` его туда и кладёт). Без привязки,
   зная ключ чужого объекта (а он виден в presigned-ссылке на скачивание), пользователь мог бы
   объявить чужой объект своим файлом — и удалить его вместе со «своей» записью.
2. **Тип** определяется по magic bytes: сервер читает первые килобайты объекта ИЗ MinIO и
   прогоняет через тот же детектор, что и буферная загрузка. Объявленный клиентом MIME — лишь
   утверждение: можно заявить `application/pdf`, а положить SVG со скриптом, который потом
   откроется в браузере по presigned-ссылке (§14.9).
3. **Размер** берётся из `statObject`, а не из тела запроса, и сверяется с лимитом категории
   определённого типа.

Любая неуспешная проверка удаляет объект: в приватном бакете не должно остаться содержимого,
за которое никто не отвечает (осиротевшие объекты — баг, см. выше).
...
- В multi-instance деплое cron выполняется только на одном инстансе. Реализовано: каждая задача обёрнута в `CronLockService.run(name, ttlMs, task)` — лок берётся через `SET NX PX`, снимается Lua-скриптом только если он всё ещё свой, TTL страхует от инстанса, умершего с локом. При недоступном Redis задача **выполняется** (fail-open): молча переставшая работать чистка хуже двойного прогона идемпотентной задачи.
- Рассылка, которая обязана дойти до каждого адресата (участники чата, участники события, получатели уведомления), не обрезается потолком: адресаты читаются страницами по курсору, и на каждую страницу ставится свой job. `jobId` включает номер страницы, `dedupeKey` — нет, иначе BullMQ отбросит все страницы кроме первой.
...
Логи отвечают на «что происходило», Sentry — на «что сломалось у конкретного пользователя».
Дублировать одно другим не нужно: **каждое** отправленное событие обязано попасть и в pino
с тем же `requestId`, чтобы из issue можно было вернуться в логи и наоборот
(`sentryEventId` в строке лога).

- `Sentry.init` — только в `src/instrument.ts`, и этот файл импортируется **первым** в
  `main.ts`. Иначе патчи OpenTelemetry не встанут на уже загруженные http/fastify/pg.
- Нет `SENTRY_DSN` → трекер не инициализируется. Так работают dev, тесты и CI; вызовы
  захвата становятся no-op'ами, поэтому `if (sentryEnabled)` в сервисах писать не нужно.
- Отправка ошибок — **только** через `common/monitoring` (`captureException` /
  `captureUnexpected`). Прямой `Sentry.captureException` в модулях запрещён: контекст
  должен быть однородным (теги `source`, `request_id`, `error_code`, `path`).
- Порог: `status >= 500`. Ожидаемые бизнес-отказы (4xx `AppException`) в трекер не идут.
- Декоратор `@SentryExceptionCaptured` **не используется**: он считает ожидаемым любой
  `HttpException`, включая 5xx `AppException`, и не даёт приложить `requestId`/`userId`.
  Захват живёт в `HttpExceptionFilter`.
- Четыре источника ошибок закрыты: HTTP (фильтр), очереди (`@OnWorkerEvent('failed')` в
  каждом процессоре), WebSocket (`ChatGateway.fail`), cron (`CronMonitorService` навешивает
  `errorHandler` на задачи — библиотека `cron` иначе глушит падение в `console.error`).
- Что НЕ уходит наружу (§14.9, §11.3): тела запросов, cookie, `Authorization`, `job.data`,
  содержимое сообщений. Из URL и имён транзакций вычищаются секреты (`?token=`,
  `/invites/:token/preview`). Реализация — `scrubSentryEvent` в `@studenthub/shared-config`
  (общая с web), тесты — `common/monitoring/sentry-scrub.spec.ts`. **Ослаблять эти тесты
  нельзя**: они и есть гарантия приватности.
- Пользователь идентифицируется только `id`. Email/ФИО в событие не кладём.
- В событие попадают строки исходного кода вокруг упавшего кадра стека
  (`contextLinesIntegration`) — ещё одна причина, почему секретов в коде быть не должно (§14.1).

---

### 13.2 Печатные QR (Ф16)

- QR ведёт на страницу приложения, а не содержит данные: наклейка не устаревает вместе с
  расписанием и работает штатной камерой телефона без установки приложения.
- Код в QR — отдельная короткая величина, НЕ `id` сущности. Так его можно перевыпустить
  (обесценив утёкшую распечатку), а сам QR получается менее плотным.
- Неизвестный код и ресурс чужого вуза дают ОДИНАКОВЫЙ `NOT_FOUND`. Разные ответы делают
  код оракулом: по нему выяснялось бы, существует ли помещение в другом вузе.
- Страницы по QR закрыты авторизацией: наклейка висит в открытом доступе, а данные за ней —
  внутренние (§14.7).
- Перевыпуск кода пишется в `AuditLog` вместе с прежним кодом: иначе непонятно, какая из
  расклеенных наклеек перестала работать.
