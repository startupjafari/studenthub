...
| 12.7 | `widgets/stats-dashboard` — chart.js через `next/dynamic` |
...
| # | Задача |
|---|---|
| 13.1 | Полный проход i18n: ru/kk/en, идентичные наборы ключей, ICU-плюрализация, перевод всех кодов ошибок |
| 13.2 | PWA: манифест, иконки, офлайн-страница, кэш-стратегии для расписания |
| 13.3 | Push-уведомления (FCM / Web Push) + настройки каналов |
| 13.4 | Playwright E2E: логин, регистрация по инвайту, лента, отправка сообщения, создание и обработка заявки, изменение расписания |
| 13.5 | Аудит безопасности по чеклисту `BACKEND_RULES §14` — вручную по каждому эндпоинту из реестра |
| 13.6 | Производительность: `EXPLAIN ANALYZE` на запросах ленты, сообщений, расписания; добавить недостающие индексы; bundle-analyzer |
| 13.7 | Доступность и адаптивность: 375/768/1280, клавиатурная навигация, контраст |
| 13.8 | ✅ Мониторинг: Sentry (`@sentry/nestjs`, `@sentry/nextjs`) — HTTP-5xx, очереди, WS, cron, SSR, браузер; чистка персональных данных перед отправкой; `/health` (terminus) и request-логи были раньше. Осталось организационное: создать проект, положить DSN, настроить алерты по error-rate |
| 13.9 | Production-инфраструктура: nginx + TLS, `docker-compose.prod.yml`, `restart: always`, бэкапы PostgreSQL и MinIO, ротация логов |
| 13.10 | Финальная документация: обновить `docs/PROJECT.md` под реальность, README со сценарием первого запуска, runbook (что делать при падении Redis/MinIO) |
...
| # | Направление | Заметка |
|---|---|---|
| 14.1 | Сторисы (24 ч, TTL, автоудаление, опросы, реакции) | Модели и cron частично заложены в Ф3. Решить, скрывать ли пункты меню в v1.0 за фича-флагом |
...
| 14.5 | Интеграции Zoom / Google Meet | Для онлайн-событий |
...
**Возможные продолжения (не сделано):** ручной ввод кода, если QR повреждён; статистика
сканирований по помещениям; наклейки с несколькими помещениями на одном листе A4 (сейчас
одно помещение = одна страница); привязка QR к отметке посещаемости.
