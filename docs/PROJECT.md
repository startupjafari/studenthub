...
| `typing:started` | `{ chatId, userId }` — в личные комнаты участников (`user:{id}`), чтобы подпись была видна и в строке списка чатов, а не только с открытым чатом. Веер ограничен 64 участниками (`TYPING_FANOUT_MAX`): в чате на сотни человек кто-то печатает всегда, и строка перестала бы что-либо значить, а отправок было бы сотни каждые три секунды. Сверх потолка — как раньше, в комнату `chat:{id}`. Членство отправителя проверяется тем же запросом, которым берутся получатели: адресация идёт мимо комнаты, и «я в неё вошёл» больше не защита |
| `typing:stopped` | `{ chatId, userId }` — тем же путём |
| `chat:updated` | `{ chat }` |
