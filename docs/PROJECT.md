...
*Что открыто мини-аппу.* Белый список `@MiniAllowed()` — 18 маршрутов: жалобы (очередь, карточка, переписка по жалобе, решение), поддержка (очередь, обращение, ответ, закрытие), состояние и рычаги платформы, сводка `GET /analytics/platform/overview`, люди (`GET /users`, `GET /users/:id`, блокировка и разблокировка) и приглашения (`GET /invites`, отзыв). Всё остальное токену с `client: 'mini'` запрещено независимо от роли. Выдачи приглашений, массового импорта и правки профиля в списке нет намеренно: они требуют ввода и выбора scope — работа для клавиатуры, а не для телефона.
