...
*Объединения дублей нет намеренно.* Второе обращение, пока первое не закрыто, не создаётся (`SupportService.open` дописывает в существующее), поэтому параллельных веток одного человека в модели просто не бывает — склеивать нечего. Механизм слияния пришлось бы заводить ради случая, который предотвращён на входе.
