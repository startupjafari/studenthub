...
**Как начинается личный чат (§50).** Вход — та же единая строка поиска над списком чатов
(Telegram-стиль): вместе с чатами и сообщениями она отдаёт секцию «Люди» из `GET /users/directory`,
клик по строке заводит переписку. Отдельного пункта «написать человеку» нет. Порядок выдачи
справочника: друзья → одногруппники → остальной вуз; в строке поиска показываются первые 8. Личный чат заводится
только внутри своего вуза; исключение — уже сложившаяся дружба (`Friendship` вуза не знает).
Роли вне вуза (платформенные) под ограничение не попадают, `EMPLOYER` справочника не получает.

Переписка с **другом** начинается сразу. Со всеми остальными — **запросом на переписку**,
и роль адресата на это не влияет: декан, староста, модератор и админ вуза получают такой же
запрос, как студент. Модель — Telegram-стиль: чат создаётся сразу, но `Chat.requestPendingForId`
хранит id адресата, который ещё не согласился. Пока он не null:
- у адресата чат виден **только** во вкладке «Запросы» — ни в «Все», ни в «Личные», ни в
  пользовательских папках, ни целью пересылки;
- уведомлений о сообщениях адресат не получает: при первом сообщении инициатора ему уходит
  одно `SYSTEM`-уведомление `chat-request` (`dedupeKey=chat-request:<chatId>`), и всё;
- инициатор может отправить не больше **5 сообщений** до принятия (`FORBIDDEN` дальше):
  без потолка непринятый запрос был бы каналом без согласия;
- пустая оболочка чата (открыли карточку, ничего не написали) адресату не показывается вовсе.

Снимается пометка тремя способами: `POST /chats/:id/request/accept`, ответное сообщение
адресата (ответ = согласие) и появление дружбы между этими двумя. `POST /chats/:id/request/decline`
удаляет чат целиком; инициатору, как и при отклонении заявки в друзья, ничего не сообщается.
...
тело `{ fileIds: string[] (1..50), reason }` → `{ urls: Record<fileId, url> }` — файлы берутся
пачкой, одно обращение = одна запись `DOCUMENT_PLATFORM_DOWNLOAD` с `metadata.fileIds`).
...
| 503 | `MAINTENANCE` | Платформа остановлена на техработы. Проходят платформенные роли и маршруты без которых режим не снять: `/auth`, `/auth/2fa`, `/mini`, `/platform`, `/health` |
...
Токен мини-аппа несёт `tfa` — признак включённой 2FA, прочитанный из БД. Без него глобальный `TwoFactorGuard` отдавал бы `403 TWO_FACTOR_SETUP_REQUIRED` на каждом рабочем запросе: платформенные роли привилегированные все. Сессия при этом выдавалась бы нормально (`/mini/session` публичный, пользователя в запросе ещё нет), и приложение выглядело бы живым при неработающих экранах. Значение не подставляется в `true`: администратор без 2FA получает тот же отказ, что и в вебе.
**Поддержка платформы** — `POST /support` (любая роль; `{ text }` → `{ id, created }`) · `GET /support` (очередь, `PLATFORM_ADMIN`/`PLATFORM_MODERATOR`, `@MiniAllowed()`) · `GET /support/:id` (обращение + переписка) · `POST /support/:id/reply` · `PATCH /support/:id/close`. Обращение — чат типа `SUPPORT_PLATFORM`: участники — автор и команда платформы. Не путать с `SUPPORT` вуза, где состоят все пользователи университета (в интерфейсе он называется «Помощь вуза»).

*Привязка и её отзыв* — `GET /mini/link` (состояние: к какому Telegram привязан аккаунт, когда привязан и когда последний раз открывали) · `DELETE /mini/link` (отзыв). Обе — `PLATFORM_ADMIN`/`PLATFORM_MODERATOR` из веба. Отзыв проставляет `TelegramAccount.revokedAt`, а строку не удаляет: перепривязка должна быть осознанным действием. Это единственный способ отобрать доступ у потерянного телефона — токен мини-аппа живёт в памяти 15 минут и сам не отзывается, а сессия по отозванной привязке больше не выдаётся.

*Язык мини-аппа* берётся из Telegram (`initDataUnsafe.user.language_code`), а не из настроек платформы: приложение открывается до выдачи сессии, и на экране привязки платформа о человеке ещё ничего не знает. Словари — `apps/mini-app/src/i18n/messages.ts` (ru/kk/en); их полнота и совпадение подстановок проверяются тестами.

*Что открыто мини-аппу.* Белый список `@MiniAllowed()` — 45 маршрутов: жалобы (очередь, карточка, переписка по жалобе, решение, квитирование, возврат в очередь, жалоба из обращения), поддержка (очередь, обращение, ответ, назначение, эскалация, закрытие, теги и их сводка, склейка дублей, голосовой ответ), состояние и рычаги платформы (включая откат последнего изменения), аналитика (`GET /analytics/platform/overview`, воронка инвайтов, размер вузов, топ действий и `complaints-latency` — из него берётся одна медиана времени разбора), люди (`GET /users`, `GET /users/:id`, `GET /users/:id/moderation`, блокировка, разблокировка и сброс сессий) и приглашения (`GET /invites`, отзыв). Всё остальное токену с `client: 'mini'` запрещено независимо от роли. Выдачи приглашений, массового импорта и правки профиля в списке нет намеренно: они требуют ввода и выбора scope — работа для клавиатуры, а не для телефона.

*Карточка для модератора.* `GET /users/:id/moderation` (Moderator+, `@MiniAllowed()`) — роль, вуз, `isBlocked`, `blockedUntil`, дата регистрации, число предупреждений и счётчик жалоб на самого человека (`total` / `upheld`). Отдельно от `GET /users/:id`: тот отдаёт профиль в полсотни полей и без `isBlocked`, а решение по жалобе или обращению принимают по четырём. Жалобы на посты и сообщения человека в счётчик не входят — собрать их значило бы пройти по всем его сущностям на каждое открытие карточки; историю по конкретной цели показывает `GET /complaints?targetId=`.

*Сброс сессий.* `PATCH /users/:id/logout` (Moderator+, `@MiniAllowed()`) завершает все сессии человека, не отбирая доступ. Угнанный аккаунт до этого останавливали блокировкой целиком — то есть наказывали пострадавшего; сброс выгоняет чужого и оставляет хозяину возможность войти заново.

*Подтверждение разрушительных действий.* Глобальный `ActionConfirmGuard` требует код 2FA в поле `code` для маршрутов с `@RequiresConfirmation()` — **только когда запрос идёт из мини-аппа** (`client: 'mini'`). В вебе человек прошёл пароль и 2FA в той же сессии, а телефон открывается одним касанием: потерянный аппарат иначе даёт полный доступ к модерации без единого вопроса. Декоратор принимает предикат по телу запроса — на `PATCH /complaints/:id/resolve` код спрашивается только при `action: 'BLOCK_USER'`: требовать его на «нарушения нет» значило бы приучить вводить код не глядя, а не требовать на блокировке через жалобу — оставить защиту декоративной. Разблокировка кода не требует: та же асимметрия, что у техработ.

Проверка живёт в guard'е, а не в сервисе, намеренно: домены `auth` и `users` замкнуты друг на друга через `forwardRef`, и каждый новый импорт в это кольцо однажды уже уронил запуск приложения при зелёных тестах и typecheck.

Сам guard зависит от **токена** `ACTION_CONFIRMATION` (`common/auth/confirmation.constants.ts` — файл без импортов), а не от класса `TwoFactorService`: прямой импорт замыкает кольцо `auth.module → guard → two-factor.service → users.service → auth.service → two-factor.service`, и `TwoFactorService` оказывается `undefined` в момент чтения метаданных `AuthService`. Тот же приём применён к `PLATFORM_STATE` для `MaintenanceGuard`. Ни типы, ни тесты этого не ловят — только реальный запуск приложения.

*Очереди задач.* `GET /platform/queues` отдаёт по каждой очереди BullMQ число ждущих и упавших задач. Растущее первое означает, что воркер не справляется, ненулевое второе — что часть работы потеряна молча; до этого оба числа были видны только в логах. Метод `QueueService.counts` возвращён вместе с экраном, который его читает, — ровно его однажды удаляли как мёртвый код, потому что читателя не было.

*Хранилище и журнал.* `GET /platform/storage` — объём файлов по записям `File`, а не свободное место на диске: S3-совместимое хранилище про свой диск не рассказывает, и обещать «осталось столько-то» было бы враньём; зато виден рост, а он и есть то, что заканчивается. `GET /platform/changes` — последние 20 изменений рычагов из журнала аудита с именами: публичное состояние авторов не отдаёт (посетителю знать незачем), а команде без ответа на «кто включил техработы» жить нельзя. Имена добираются отдельным запросом — у `AuditLog` нет связи с `User` намеренно, журнал переживает удаление аккаунта.

*Наблюдение за зависимостями.* Cron `watchServices` (каждые 5 минут) проверяет те же три зависимости, что и `/health`, и сигналит **только на переход** «упало» / «поднялось»: повторять «всё ещё лежит» каждые пять минут бессмысленно, а поток одинаковых сообщений учит их не читать. Сигнал идёт как эскалация — мимо дежурства и тихих часов.

*Всплеск ошибок.* `HttpExceptionFilter` считает 5xx по минутным корзинам в Redis (`recordServerError`), тот же cron читает сумму за час и уведомляет при 25+, молча следующий час. Это возвращение счётчика, удалённого в начале эпика как мёртвый код, — с двумя отличиями: считаются только серверные ошибки (единицы в сутки, а не тысячи в минуту) и у счётчика есть читатель.

Второе обращение при открытом первом не заводится — дописывается в существующее (`created: false`): иначе один человек за минуту создаёт десяток веток, и очередь перестаёт показывать, сколько людей ждут ответа. Закрытие — отметка времени `Chat.supportClosedAt`; переписка остаётся, а **ответ в закрытое обращение открывает его снова**, иначе человек с уточняющим вопросом завёл бы второе обращение о том же. Сотрудник, назначенный после создания обращения, добавляется в участников лениво при открытии очереди. О чужом обращении сервер отвечает `NOT_FOUND`, а не `FORBIDDEN`: по разнице ответов перебором id вычислялось бы, кто обращался в поддержку.

*Назначение.* `PATCH /support/:id/assign?take=true|false` (`@MiniAllowed()`); очередь фильтруется `?assignee=any|mine|free`. Перехватить чужое обращение нельзя: условие `supportAssigneeId: null` в самом запросе не даст двум одновременным «взять» победить обоим, и второй получает `409 CONFLICT`. Отдать обратно можно только своё.

*Теги.* `PATCH /support/:id/tags` (staff, `@MiniAllowed()`; набор заменяется целиком, до трёх) и `GET /support/tags` — счётчики за 30 дней. Список закрытый (`SUPPORT_TAGS`: ACCESS, DATA, DOCS, BUG, FEATURE, ABUSE, OTHER): свободные теги за неделю разрастаются в «оценки», «оценка», «Оценки!», и сводка «о чём спрашивают чаще» перестаёт складываться. Ради этой сводки теги и заведены — «поддержка отвечает на одно и то же» превращается в «шестьдесят обращений про доступ за месяц», то есть в задачу продукту. Очередь фильтруется `?tag=` на сервере: «все обращения про доступ» среди тридцати загруженных строк — это не «все». Окно в 30 дней намеренное: за всё время счётчик показывал бы историю платформы, а не то, что происходит сейчас.

*Голосовой ответ.* `POST /support/:id/voice` (staff, `@MiniAllowed()`, multipart, одно аудио). На телефоне надиктовать быстрее, чем набрать, а поддержка с телефона только этим и занимается. Отдельная ручка, а не `@MiniAllowed()` на `POST /chats/:id/messages`: последнее открыло бы токену мини-аппа все чаты сотрудника, включая личные. Файл именуется `voice-msg.<ext>` — по имени чат распознаёт голосовое, потому что mime по содержимому непредсказуем (webm-аудио браузеры отдают как `video/webm`). Правила текстового ответа действуют те же: закрытое обращение открывается снова, отметка первого ответа ставится один раз.

*Склейка дублей.* `POST /support/:id/merge` (staff, `@MiniAllowed()`; тело `{ intoId }`). Параллельных веток одного человека модель не допускает (`open` дописывает в открытое), но последовательные бывают: обращение закрыли, человек завёл новое о том же. Склеенная ветка закрывается и получает указатель `support_merged_into_id`; **сообщения не переносятся** — перенос переписал бы отправителям их собственную историю, а порядок внутри чата задан `seq`, который в чужом чате ничего не значит. Вместо переноса `GET /support/:id` отдаёт переписку целевой ветки вместе с перепиской склеенных, отсортированную по времени. Склеивать можно только обращения одного человека: две ветки разных людей — это чужая переписка в чужом обращении. Цепочки запрещены (`409`): читатель идёт ровно на один шаг, и ветка, склеенная в склеенное, не была бы видна нигде. В очереди склеенные не показываются ни среди открытых, ни среди закрытых, а в ответе треда есть `siblings` — другие обращения того же человека, то есть готовые кандидаты на склейку.

*Поиск и автозакрытие.* `GET /support?search=` ищет сразу по тексту переписки и по фамилии автора: «мы это уже кому-то отвечали» помнят словами, «что там было у Сериковой» — фамилией, и заставлять выбирать между ними незачем. Ежедневный cron `closeStaleTickets` закрывает обращения без движения 14 дней, но **только те, где последним писала команда**: обращение, где последним писал автор, — это неотвеченный вопрос, и закрывать его по таймеру значит прятать собственный долг. Ответ закрытое обращение открывает снова, поэтому закрытие ничего не отнимает.

*Эскалация.* `POST /support/:id/escalate` — действие, а не состояние: ответ на эскалацию человек, а не флаг в таблице. Уведомление уходит только `PLATFORM_ADMIN` и **мимо дежурства и тихих часов**: эскалируют ровно тогда, когда обычный путь не сработал, и глушить её теми же правилами значило бы глушить именно тот сигнал, ради которого её завели.

*Время первого ответа* (`Chat.supportFirstReplyAt`) проставляется один раз, при первом ответе КОМАНДЫ, и больше не меняется: повторное открытие обращения не делает первый ответ быстрее, а перезапись улучшала бы метрику задним числом.

**Уведомления в Telegram** — `TelegramNotifyService` (`common/telegram`) пишет команде платформы о срочной жалобе (приоритет `HIGH`) и о новом обращении. Глобальный модуль без зависимостей: уведомлять нужно из жалоб и из поддержки, а импорт `MiniModule` втянул бы в оба домена `AuthModule`. Кнопка в сообщении открывает нужную карточку через `startapp=complaint_<id>` / `support_<id>` — нужен `MINI_APP_URL`, без него уходит голый текст. Текст уведомления не содержит ни жалобы, ни обращения: Telegram — чужая инфраструктура, и сообщение говорит, ЧТО пришло, а читается оно в мини-аппе. Ошибка отправки никогда не роняет операцию, без `TELEGRAM_BOT_TOKEN` сервис молчит.

**Состояние платформы** — `GET /platform/state` (**публ.**) → `{ maintenance, banner, disabledSections, announcedVersion, season }`. Глобальные рычаги, которыми админ управляет вебом без деплоя: режим техработ, баннер-объявление, погашенные разделы, объявленная версия релиза. Хранятся в `platform_state` (одна строка, `id = "singleton"`), кэш в Redis на 60 секунд; источник правды — БД, чтобы перезапуск Redis не снимал режим техработ.

Публичен вынужденно: страница логина тоже обязана показать техработы, иначе человек бьётся в форму, которая всё равно не пустит. Наружу уходят только объявления, адресованные посетителю; `updatedById` не публикуется.

Режим техработ и баннер хранятся **сроком** (`maintenance_until`, `banner_until`), а не флагом: состояние «включено навсегда» выразить нельзя, поэтому забытый выключатель снимается сам и cron для этого не нужен. Срок применяется на сервере — наружу уходит либо действующее объявление, либо `null`, и клиенты с уехавшими часами не решают этот вопрос каждый по-своему. Текст отдаётся только на всех трёх языках сразу: строка на двух из трёх — дыра в интерфейсе.

*Плановые техработы.* `startsInMinutes` задаёт отложенное начало (`maintenance_from`); срок окончания считается от НАЧАЛА окна, а не от нажатия, иначе работы, назначенные на вечер, кончались бы через час. До начала окна платформа работает, `MaintenanceGuard` пропускает, а веб показывает предупреждение: предупреждение и остановка — разные состояния одного события, и задаются они одним действием, чтобы два объявления об одном событии не разошлись.

*Адресный баннер.* `banner_roles` и `banner_university_ids` (пустые массивы — всем) фильтрует КЛИЕНТ: ответ `GET /platform/state` публичный и общий, делать его персональным значило бы отказаться от кэша ради объявления, которое и так не секрет. Роль и вуз проверяются как пересечение: «преподавателям такого-то вуза» не должно долетать до преподавателей остальных. Снятие баннера стирает и прицел — иначе следующий унаследовал бы чужой.
...
*Праздничное оформление.* `season` → `{ off, override }`. Сам праздник веб считает по справочнику в своём бандле (`apps/web/src/shared/config/holidays.ts`): даты одинаковы для всех и меняются раз в несколько лет законом, ради них не нужен ни запрос, ни таблица — и оформление обязано ехать той же сборкой, что и код, который его рисует. В `platform_state` лежит только вмешательство человека: `season_off` гасит праздник у всех без деплоя (траур, авария), `season_override` показывает сезон вне его даты (юбилей вуза, день открытых дверей). Срока у сезона нет намеренно, в отличие от техработ и баннера: те гасят продукт или требуют внимания, а забытый сезон — это другой цвет кнопки. Допустимые значения `override` перечислены в `SEASON_IDS` (`shared-schemas`); совпадение списка со справочником сторожит тест в вебе. Личная настройка человека сильнее подмены: согласие видеть оформление он даёт сам.

*Запись рычагов* — `PATCH /platform/maintenance` · `PATCH /platform/banner` · `PATCH /platform/sections` · `PATCH /platform/season` · `PATCH /platform/release`. Все — только `PLATFORM_ADMIN`, все помечены `@MiniAllowed()` (управление платформой с телефона и есть смысл мини-аппа). Модератора среди пишущих нет: его работа — разбирать жалобы, а выключатель, гасящий продукт для всех, к ней не относится. Каждое изменение пишется в `AuditLog` (`platform.maintenance.on|off`, `platform.banner.on|off`, `platform.sections.set`, `platform.season.set`, `platform.release.announce`).
...
*Принуждение.* Глобальный `MaintenanceGuard` (сразу после `JwtAuthGuard`) отдаёт `503 MAINTENANCE` всем, кроме `PLATFORM_ADMIN`/`PLATFORM_MODERATOR` и маршрутов с `@MaintenanceExempt()`. Список исключений — это список того, без чего режим НЕЛЬЗЯ СНЯТЬ, поэтому декоратор ставится на весь контроллер, а не на отдельные ручки: вход (`AuthController`, `TwoFactorController`), мини-апп (`MiniController`), состояние платформы (`PlatformController`) и живость (`HealthController` — без неё хостинг решит, что контейнер умер, и будет перезапускать его по кругу).

*Дежурство по очереди* — `GET /platform/duty` · `PATCH /platform/duty` (`PLATFORM_ADMIN`, `@MiniAllowed()`; `rotation` — до 10 id по порядку). Крон `rotateDuty` по понедельникам в 9:00 делает дежурным следующего за текущим. Позиция ищется по id текущего дежурного, а не хранится числом: номер смены пришлось бы чинить руками при каждой правке списка, а по имени всё сходится само; дежурный не из списка (назначили руками на выходные) начинает круг сначала. Пул — те, кто привязал Telegram (`GET /mini/links`): дежурить может только тот, кому бот в принципе может написать. Отдельно от публичного `GET /platform/state` намеренно — список всей команды посетителю сайта знать незачем. Ротацию зовёт CleanupService через токен `PLATFORM_STATE`, а не через `PlatformService`: импорт сервиса втянул бы в планировщик домен auth.

*Откат* — `POST /platform/undo` (`PLATFORM_ADMIN`, `@MiniAllowed()`). Возвращает состояние, которое было до последнего изменения рычагов: чипы на телефоне стоят рядом, палец один, а прежнее состояние нигде не показано. Снимок «как было» пишется в метаданные каждой записи `platform.*` и содержит **только переписанные поля** — возврат баннера не снимет техработы, включённые в ту же минуту кем-то другим. Запись об откате сама несёт снимок, поэтому второе нажатие возвращает всё обратно. Два ограничения: старше 30 минут — `409` (это уже не промах, а решение), и откатом нельзя включить техработы — на это есть своя кнопка, спрашивающая код 2FA.

*Уведомления команде* — `PATCH /platform/notifications` (`PLATFORM_ADMIN`, `@MiniAllowed()`): тихие часы, виды уведомлений, дежурный и час суточной сводки. Хранятся в той же строке `platform_state`.

Тихие часы задаются целыми часами по времени сервера и почти всегда переходят через полночь («с 22 до 8»), поэтому проверка — не «между», а «вне диапазона» при `from > to`. `from === to` означает тишину круглые сутки: так уведомления выключают, не стирая настройку.

`muted_notifications` — список ВЫКЛЮЧЕННОГО (`complaint|ticket|reply|digest`), чтобы новый вид уведомления по умолчанию доходил. Заданный `duty_user_id` направляет уведомления только дежурному: сообщение всей команде означает, что не среагирует никто. Отказ чтения настроек уведомления не глушит — они не условие доступа.

Суточная сводка идёт ежечасным cron `sendDailyDigest` (`CleanupService`, §9.3) и отправляется в выбранный час: держать время в cron-выражении значило бы перезапускать приложение ради его смены. Сводка уходит и при пустой очереди — по её отсутствию нельзя отличить спокойный день от сломавшейся отправки.

Вопрос «идут ли техработы» задаёт каждый запрос, поэтому ответ лежит в памяти процесса 5 секунд (`MAINTENANCE_MEMO_MS`) — иначе к каждому запросу платформы добавился бы round-trip в Redis. Столько же режим доходит до инстанса, который его не включал; инстанс, принявший команду, сбрасывает память сразу.

*Разделы в вебе.* Погашенный раздел уходит из навигации (`isNavKeyDisabled`) и закрывается по прямой ссылке (`disabledSectionForPath` в `PlatformGate`) — **у всех, включая платформенные роли**: решающий, вернуть ли раздел, должен видеть ту же платформу, что и пользователи. Административные экраны настройки (типы документов, доступы) не гасятся: раздел выключают как раз тогда, когда в нём что-то сломалось.

Токен мини-аппа несёт `client: 'mini'` и живёт 15 минут; refresh-токена нет — клиент присылает свежий `initData`. Такой токен допускается **только** на маршруты с `@MiniAllowed()` (`MiniAppGuard`), на остальных — `403 FORBIDDEN`, независимо от роли. Состав белого списка — в §8.3 («Что открыто мини-аппу»): держать два перечня в разных разделах значит однажды их разъехать.
...
**Пользователи** — `GET|PATCH /users/me` · `PATCH /users/me/username` (смена имени входа; тело `{ username }`, нормализуется в нижний регистр, `409 USERNAME_TAKEN` если занято; пишется в аудит как `change_username`) · `POST|DELETE /users/me/avatar` · `POST|DELETE /users/me/cover` (обложка профиля, multipart-изображение ≤ 10 МБ, бакет `profile-covers`) · `PATCH /users/me/password` · `DELETE /users/me` · `GET /users/:id` · `GET /users` (Admin+; фильтры `role/facultyId/groupId/search/blocked`, offset-пагинация (`limit` ≤ 200 — таблица даёт выбрать 20/100/150/200) + сортировка `?sort=name|email|role|blocked|createdAt&order=asc|desc` — по всей выборке, не по странице; поле сортировки — enum, произвольное имя в `orderBy` не попадает) · `GET /users/directory?q=&limit=` (справочник людей «кому написать», §50 — **все роли**, в отличие от `GET /users`: отдаёт только визитки (id, ФИО, аватар, роль, headline, группа/факультет) в пределах вуза смотрящего — платформенные роли вуза не имеют и ищут по всем, `EMPLOYER` доступа к справочнику не имеет. Ответ `{ items, hasMore }` без курсора: это пикер, при переполнении клиент просит уточнить запрос. У каждой карточки `isFriend` и `section` = `friend|group|university` — секции задают порядок: друзья → одногруппники → остальной вуз. Заблокированные в любую сторону исключены) · `GET /users/export?format=xlsx|csv&locale=` (Admin+; выгрузка списка файлом — та же выборка и те же фильтры, что у `GET /users`, включая scope; потолок 5000 строк, выше — `400 BAD_REQUEST` вместо тихого обрезания; пишется в `AuditLog` (`users_export`) и в `DocumentExport`) · `PATCH /users/:id/block|unblock` (Moderator+). Профиль отдаёт `avatarUrl`, `avatarThumbUrl` (квадратное превью ≈128px, генерируется джобой `generate-thumbnail` в очереди `file-processing`; асинхронно, до готовности `null`) и `coverUrl` (публичные URL; `coverUrl` виден и в «визитке» закрытого профиля).
...
**Файлы** — `POST /files/upload?bucket=` (буферная, ≤10 МБ) · `POST /files/presign` + `POST /files/confirm` (прямая загрузка крупных файлов: шаги 1 и 3, между ними браузер делает PUT в MinIO) · `GET /files/:id/presigned` · `DELETE /files/:id`. Доменные загрузки имеют свои пары: `POST /documents/upload/presign|confirm`, `POST /materials/:id/files/presign|confirm`, `POST /profile-content/media/presign|confirm`. Файлы больше `FILE_UPLOAD.MULTIPART_THRESHOLD_BYTES` (50 МБ) — многочастной загрузкой: `POST /files/multipart/start` (→ `key`, `uploadId`, размер и число частей) · `POST /files/multipart/urls` (подписи на диапазон частей `[from, to]`, нумерация с единицы) · PUT каждой части браузером · `POST /files/multipart/complete` (сервер склеивает и прогоняет объект через ту же проверку, что и `confirm`) · `POST /files/multipart/abort`. Сборку делает сервер, а не подписанная ссылка: иначе в приватном бакете появлялись бы объекты, которых никто не проверял по magic bytes. `etag` частей присылает клиент — в `minio@8` `listParts` объявлен protected, и серверу их взять неоткуда; подлог бесполезен, MinIO сверяет их сам. Ключ на каждом шаге обязан начинаться с идентификатора вызывающего. Предел видео и документов — 500 МБ (скан сборника или лекция с телефона весят больше прежних 25 и 100 МБ), изображения и аудио прежние. Правила недоверия на шаге confirm — `docs/BACKEND_RULES.md §8.1`.
...
**Заявки (услуги)** — каталог: `GET /application-categories` (категории с доступными услугами) · `GET /application-services/:id` (детали: требования-документы + поля формы). Заявки: `POST /applications` (черновик по `serviceId`) · `PATCH /applications/:id` (правка черновика: `deliveryType`+`formData`, только владелец/DRAFT) · `POST /applications/:id/submit` (DRAFT→SUBMITTED: номер SH-YYYY-N + `dueAt` по SLA + валидация формы) · `POST /applications/:id/cancel` (→CANCELLED, владелец до подготовки) · `GET /applications` (список/очередь: server-side пагинация + фильтры `status/serviceId/categoryCode/facultyId/assignedToId/search/overdue/dueToday`, scope через `ApplicationPolicy`) · `GET /applications/:id` (детали + timeline, scope-гейт). Обработка и выдача реализованы: `POST /applications/:id/take|assign|request-correction|start-preparation|reject` · `POST /applications/:id/results` (результат: `type` + `documentNumber?`/`note?`; с `fileId`+`documentType` сервер сам заводит Document на имя СТУДЕНТА — `ownerId=studentId`, `category=ISSUED_BY_UNIVERSITY`, `issuedByUniversity=true` — через привилегированный `DocumentsService.issueToOwner`, публичного эндпоинта у него нет) · `POST /applications/:id/results/certificate?locale=ru|kk|en` (на этапе `IN_PREPARATION` платформа САМА формирует справку об обучении по данным профиля студента вместо загрузки файла сотрудником: регистрирует документ в `DocumentExport`, печатает код проверки и QR в бланк, заводит `Document` на имя студента (тип `STUDY_PLACE_REF`) и записывает результат заявки. Возвращает `{ documentId, verificationCode }`. Сбой записи в журнал ОТМЕНЯЕТ выдачу: документ без записи непроверяем) · `POST /applications/:id/results/certificate/revoke` (`{ reason? }` — отозвать выданную справку: файл у студента остаётся, но проверка по коду с этого момента отдаёт `REVOKED`. Право — то же, что на обработку заявки; повторный отзыв → `404`, чтобы «уже отозвано» не выглядело успехом. Пишется событием `RESULT_REVOKED` в историю заявки) · `GET /applications/:id/results/:resultId/url?download=1` (presigned-ссылка на выданный документ; гейт — владелец заявки или обработчик, владение Document не проверяется) · `POST /applications/:id/mark-ready|issue|deliver` · документы заявки — `POST /applications/:id/documents`, `POST /applications/:id/documents/:docId/accept|request-replacement`, `GET /applications/:id/documents/:docId/url`. Права — `ApplicationPolicy` (роль+scope, единый источник).

**Посты** — у публикации есть постоянный адрес `/posts/<id>` (страница поста; видимость решает сервер — невидимый отвечает NOT_FOUND) и необязательный `title` (заголовок) и `content` в ОГРАНИЧЕННОМ markdown: `**жирный**`, `*курсив*`, `~~зачёркнутый~~`, `` `код` ``, `[ссылка](url)`, маркированный и нумерованный списки, цитата. Разметка хранится как есть, разбирается на клиенте компонентом `shared/ui/markdown` в React-элементы — `dangerouslySetInnerHTML` не используется нигде, схемы ссылок ограничены http/https/mailto. — `GET|POST /posts` · `PATCH /posts/:id` (правка СВОЕЙ публикации: только `title` и `content`; модератору правка чужого текста не положена — в отличие от удаления) (лента — cursor по видимости; таб `filter=ALL|GROUP|UNIVERSITY|TEACHERS|IMPORTANT|SAVED` сужает поверх видимости: GROUP/UNIVERSITY — по audience, TEACHERS — посты от преподавателей, IMPORTANT — закреплённые, SAVED — личная полка зрителя) · `GET|DELETE /posts/:id` (удаление — автор/модератор scope) · `POST /posts/:id/publish` (автор публикует СВОЙ черновик: DRAFT → PUBLISHED с `publishedAt`; не черновик → 400 `BAD_REQUEST`, чужой → 403; отложенный публикует крон по `scheduledAt`, досрочно — нельзя) · `PATCH /posts/:id/pin` (роль строго выше автора) · `POST /posts/:id/reactions` · `DELETE /posts/:id/reactions/:emoji` · `GET|POST /posts/:id/comments` · `DELETE /posts/:id/comments/:commentId` · `POST /posts/:id/repost` (только опубликованный и не личный пост: PERSONAL → 403 `FORBIDDEN`, DRAFT/SCHEDULED → 400 `BAD_REQUEST`; аудитория репоста своя, от оригинала не наследуется; репост репоста ссылается на первоисточник)

*Избранное.* `POST /posts/:id/bookmark` переключает личную закладку и возвращает `{ bookmarked }`. Переключатель, а не две ручки: клиент не обязан знать текущее состояние, чтобы нажать кнопку, иначе два быстрых нажатия подряд расходятся с сервером. Таблица `post_bookmarks` (пара `user_id + post_id`, уникальная) — своя, а не общая с закладкой на статью профиля (`bookmarks`): это другой домен, и одна таблица на два означала бы полиморфную ссылку.

Признак **личный и наружу не уходит**: в ответе поста есть только `bookmarked` для зрителя, число сохранивших не считается и не публикуется — в отличие от реакций, которые видны всем намеренно. Считается он отдельным запросом на страницу выдачи, а не включением отношения в выборку: отношение отдало бы, КТО ещё сохранил пост. Полка (`filter=SAVED`) пересекается с видимостью, как и любой другой таб: пост, который человек больше не имеет права видеть, в ней не появится.
...
**Задания** (Academic Core, задача 3) — `GET /assignments` (по роли; фильтры `courseId/groupId/status/mine`, offset-пагинация; студент/староста видят только PUBLISHED/CLOSED своей группы + своё поле `mySubmission`; преподаватель — свои дисциплины; декан/админ — scope) · `GET /assignments/:id` · `POST /assignments` (препод./декан/админ, только своя дисциплина) · `PATCH /assignments/:id` · `POST /assignments/:id/publish|close` · `DELETE /assignments/:id` · `GET /assignments/:id/submissions` (workspace проверки) · `GET /assignments/review-queue?limit=` (препод./декан/админ: очередь проверки — `{ total, items[{id,title,subject,dueAt,pending}] }`, сдано и ждёт оценки, в том же scope, что и список заданий. Отдельный агрегат, потому что список заданий счётчика сданных работ не отдаёт: иначе клиенту пришлось бы опрашивать сабмиты каждого задания по очереди). Сдача студента: `PUT /assignments/:id/submission` (черновик: text/linkUrl) · `POST /assignments/:id/submit` (DRAFT/RETURNED→SUBMITTED, проверка срока/allowLate). Проверка: `POST /submissions/:id/grade` (score+feedback→GRADED) · `POST /submissions/:id/return` (feedback→RETURNED). Модели: `Assignment` (`courseId`, type/submissionType/status строками, maxScore/maxAttempts/allowLate/publishAt/dueAt), `Submission` (`@@unique([assignmentId,studentId])`, status DRAFT|SUBMITTED|GRADED|RETURNED, text/linkUrl/attemptNumber/score/feedback). Файловые вложения сдач — следующей миграцией (сейчас текст + ссылка).

**Консультации** (Academic Core, задача 15) — `GET /consultations/mine?page=&limit=&sort=&order=` (препод — свои слоты, студент — свои записи; **offset-пагинация + сортировка на сервере**: ответ `Paginated` — строки в `data`, число в `meta.total`; `sort` = `startsAt|status|student`, `order` = `asc|desc`, по умолчанию `startsAt asc`; `from=` — только слоты не раньше момента (нужен дашборду: список по возрастанию, и без фильтра первая страница — самые старые). Сортировка серверная, потому что клиент видит одну страницу: упорядочивание в браузере переставляло бы только её. Сортировка по студенту идёт по фамилии и имени, а не по `studentId`) · `GET /consultations/teachers` (студент: преподаватели вуза с открытыми слотами) · `GET /consultations/slots?teacherId=` (открытые слоты препода для записи) · `POST /consultations/slots` (препод/декан: интервал приёма) · `DELETE /consultations/slots/:id` · `POST /consultations/slots/:id/book` (студент, `{topic?}`) · `POST /consultations/slots/:id/cancel` (студент — снять запись, препод — отменить слот). Статусы: `OPEN|BOOKED|CANCELLED`. Модель `ConsultationSlot` (teacherId/studentId?/startsAt/endsAt/location?/isOnline/topic?). Уведомления (SYSTEM) на запись/отмену.
...
**Цифровой студенческий** (Academic Core, задача 20) — `GET /student-id/me` (студент/староста: карта — ФИО/фото/дата рождения/университет/факультет/специальность/группа/курс/уровень/форма обучения/финансирование/общежитие/год поступления/год выпуска/№ билета/статус + подписанный QR TTL 5мин со ссылкой `/verify-id?t=`; справочные поля (`educationLevel`, `studyForm`, `fundingType`, `academicStatus`) отдаются кодами — `BACHELOR`, `FULL_TIME`, `GRANT`, `ACTIVE` — и переводятся на фронте) · `GET /student-id/verify?token=` (сотрудник вуза: верификация карты по QR, scope — тот же вуз). Без новой модели (данные из `User`). Токен — HMAC (`common/crypto/signed-token`).
...
- `activity-heatmap` — сетка 7×24 событий аудита, `cells[dow][hour]`, `dow` 0 = понедельник,
  плюс `days[dow]` — сколько календарных ДАТ этого дня недели попало в период, и `tz` эхом.
  Часы раскладываются в зоне query-параметра **`tz`** (IANA-имя, по умолчанию `UTC`): единого
  локального часа у платформы нет (у вузов свои зоны), поэтому берётся зона смотрящего —
  «пик в 18 UTC» для Алматы это 23:00, и без перевода картина рабочего дня сдвинута на сутки.
  `days` — делитель: в 30-дневном окне понедельников пять, а суббот четыре, и без него
  «в субботу меньше» означало бы лишь «суббот было меньше». Дашборд строит на этом суточный
  профиль (будни / выходные, среднее за день) и ряд по дням недели.
...
**Выгрузки и проверка документов** — `GET /verify/:code` (**публ.**, лимит 20/мин). Всё, что пользователь скачивает, несёт происхождение: метаданные файла (`Title`/`Author=StudentHub`/`Creator=StudentHub v{версия}`/`Producer=StudentHub — {домен}`), видимая шапка «Сформировано {дата, время, таймзона} · Пользователь: {ФИО}», колонтитул с доменом и «стр. N из M», единое имя файла `studenthub_{вид}_{YYYY-MM-DD}.{ext}` и `Content-Disposition` с обеими формами имени (RFC 5987). Собирает это `ExportBrandingService` (`common/export`) — константы в контроллерах запрещены: домен временный, и переезд обязан быть правкой одной переменной. Язык подписей — `?locale=ru|kk|en` (на бэкенде i18n-слоя нет, словарь брендирования живёт в модуле; значения из БД при этом не переводятся).

Excel: книга из двух листов — **«Данные» первым** (таблица строго с A1, заголовки в первой строке, автофильтр, именованный диапазон `StudentHub_Data`) и «Инфо» вторым (система, домен, тип отчёта, дата с таймзоной, кто выгрузил, версия, фильтры). Шапку над таблицей не ставим: она ломает любой импорт «первая строка = заголовки», а `read_excel` без указания листа берёт первый. Закрепление строки в SheetJS 0.18.5 при записи не поддерживается — вместо него автофильтр. В CSV листов нет, поэтому происхождение несут только имя файла и журнал.

Журнал `DocumentExport`: `shortId` (8 символов, алфавит без 0/O и 1/I/L — код читают с бумаги), `userId`, `kind`, `format`, `params` (условия выборки, без ПД), `rows`, `ip`, `userAgent`, `createdAt`, `revokedAt` + реквизиты официального документа снимком на момент выдачи (`subjectName`, `documentNumber`, `issuerName`). Отдельно от `AuditLog` по двум причинам: там нет публичного идентификатора, и cron `cleanAuditLogs` чистит записи старше 90 дней — а выданную справку проверяют годами. Сбой записи журнала **не** отменяет рабочую выгрузку (принцип `AuditService`), но **отменяет** выдачу официального документа.

Страница проверки отвечает на один вопрос — «документ настоящий и действует?». Отдаёт тип, номер, кем выдан, дату, статус `VALID|REVOKED` и ФИО в виде «Фамилия И.О.»: код подбираем, и раздавать по нему полные данные нельзя. Ответ на «кода нет» и на чужой вид выгрузки одинаковый. Отзыв — `POST /applications/:id/results/certificate/revoke`: документ на руках не изымается, меняется только статус в журнале. Рабочие выгрузки в журнале тоже есть, но зелёной отметки не получают — страница говорит «служебная выгрузка, не официальный документ». Для казахстанской ЭЦП это основной путь проверки для человека: обычные читалки PDF ГОСТ-подпись не понимают и показывают «подпись недействительна».

**Чаты** — `GET|POST /chats` (список: cursor-пагинация `?cursor=&limit=` (limit ≤ 200, дефолт 100), порядок `updatedAt desc, id desc`; «закреплённые сверху» собирает клиент по `pinnedAt` поверх всех страниц — внутри одной страницы сквозной порядок не получить. Последнее сообщение + флаги непрочитанного/`muted`; официальные чаты авто-создаются; `POST` с `type=PRIVATE` требует **ровно один** `memberIds` (иначе `400`: проверки блокировки/вуза/дружбы делаются для собеседника, а не для списка) и **идемпотентен по паре участников** — существующий личный чат возвращается, а не создаётся второй, включая скрытый «у себя» (`hiddenAt`); `type=GROUP` создаёт всегда — группу заводят явно, со своим названием) · `GET /chats/:id/messages` (cursor; аддитивно: `around=<messageId>` — окно из ≤`limit` старее + целевое + ≤`limit` новее, `meta.{cursor,hasNext}` — старые, `meta.{prevCursor,hasPrev}` — новые; `aroundDate=<ISO>` — окно вокруг первого сообщения на/после даты (переход по дате; нет таких — новейшие); `direction=older|newer` — направление курсорной подгрузки, `newer` для скролла вниз после jump) · `POST /chats/:id/messages` (multipart: текст + вложения, `message:new` эмитится сервером один раз) · `GET /chats/:id/updates?since=<seq>&sinceTs=<ISO>` (догон после обрыва связи: `created` — сообщения с `Message.seq > since`, `mutated` — известные клиенту сообщения с правкой или (от)закреплением после `sinceTs`, `deletedIds` — удалённые за то же время, `latestSeq` — текущий `Chat.lastSeq`; `overflow: true` — разрыв больше 200 сообщений, клиент перезапрашивает историю целиком. Без `sinceTs` возвращаются только новые. Снятие реакции дельтой не покрывается — удалённая строка `MessageReaction` следа не оставляет) · `GET /chats/search?q=&chatId=&senderId=&hasFile=` (в чате при `chatId`, иначе по всем чатам участника; фильтры §4: `senderId` — по автору, `hasFile` — только с вложениями; cursor) · `GET /chats/:id/media?type=media|file|voice` (общие материалы §23: вложения по MIME — media=image/video, voice=audio, file=остальное; cursor; отдаёт `id,messageId,name,mime,size,hasPoster,createdAt,sender`) · `GET /chats/:id/links` (сообщения с `linkPreview`; cursor; `messageId,createdAt,sender,linkPreview`) · `POST /chats/:id/poll` (опрос §38: сообщение-опрос + `ChatPoll`; `{question, options[2..10], multiple?, anonymous?, allowRevote?, randomOrder?}`; эмит `message:new`) · `GET /chats/polls/:pollId` (результаты для смотрящего: счётчики + `myOptionIds`; у неанонимного опроса — `options[].voters` (имена и аватары первых 300 проголосовавших, §39), у анонимного список всегда пуст — личности не раскрываются даже автору) · `POST /chats/polls/:pollId/vote` (`{optionIds[]}`, пустой — снять голос; эмит WS `poll:updated {pollId, chatId}`) · `GET /chats/:id/pinned` · `POST|DELETE /chats/messages/:messageId/pin` · `POST /chats/messages/:messageId/reactions` (тоггл эмодзи) · `POST /chats/:id/forward {messageId}` (пересылка в этот чат, участник обоих) · `GET /chats/:id/export` (история хронологически, cap 5000; JSON-массив — контракт клиента) · `GET /chats/:id/export/file?format=txt|json&locale=` (то же файлом: `txt` — расшифровка с шапкой происхождения, `json` — `{ export, messages }`, где `export` содержит машинные ключи; отдельный маршрут, потому что файл уходит в `reply` мимо интерцептора ответа) · `POST /chats/:id/request/accept` · `POST /chats/:id/request/decline` (§50, запрос на переписку: принять — `Chat.requestPendingForId` обнуляется и чат становится обычным; отклонить — чат удаляется целиком у обеих сторон, инициатору отказ не сообщается. Решение принимает только адресат: инициатору — `FORBIDDEN`, по уже принятому — `CONFLICT`) · `GET /chats/unread` (сводка `{chats, messages}` для бейджа в навигации — отдельный лёгкий запрос: список чатов ради числа не тянут; архив и непринятые запросы в счёт не идут) · `POST|DELETE /chats/:id/archive` (архив «у себя»: чат уходит в отдельную вкладку и перестаёт считаться в бейдже; новое сообщение архив НЕ снимает — иначе официальные чаты возвращались бы каждый день. Поле `ChatMember.archivedAt`, флаг `archived` в списке) · `GET|POST /chats/:id/scheduled` · `PATCH|DELETE /chats/scheduled/:id` (отложенные сообщения: модель `ScheduledMessage` — отдельная, а не поле на `Message`, потому что занятый заранее `seq` означал бы дыру в ленте до доставки. Только текст, без вложений; до 50 в очереди на чат; доставляет минутный cron `deliverScheduledMessages` в `CleanupService` (§9.3 — единственный дом расписаний), антифлуд к доставке не применяется, а неудачу автор получает `SYSTEM`-уведомлением, а не молчанием) · `POST|DELETE /chats/:id/mute` (§17: POST `{minutes?, importantOnly?}` — заглушить на время (`ChatMember.mutedUntil`), иначе навсегда; `importantOnly` — режим «только важные»: ответы на мои сообщения и упоминания меня по имени уведомление всё равно создают (`ChatMember.muteImportantOnly`); DELETE — включить, флаг режима сбрасывается) · `GET|POST /chats/folders` · `PATCH|DELETE /chats/folders/:id` (§2, пользовательские папки: личные вкладки поверх списка чатов; состав правится целиком, до 20 папок и 200 чатов в каждой; встроенные вкладки по типу чата считает клиент и в БД их нет) · `POST|DELETE /chats/:id/pin` (закрепить/открепить чат «у себя» — сверху списка, персонально) · `GET /chats/:id/presence` (онлайн-статусы участников) · `GET /chats/:id/members` (участники с ролью, онлайн, `lastSeenAt` §49 у оффлайн, и флагом `banned` — окно управления группой) · `GET /chats/attachments/:fileId/url` (presigned, доступ по членству) · `POST /chats/:id/members` · `DELETE /chats/:id/members/:userId` · `POST|DELETE /chats/:id/members/:userId/ban` (бан/разбан участника группы — только создатель) · `POST|DELETE /chats/:id/avatar` (аватар группы, multipart; только создатель) · `POST|DELETE /chats/blocks/:userId` (личная блокировка — запрет переписки в PRIVATE в любую сторону) · `GET /chats/blocks` (список заблокированных мной) · `PATCH /chats/:id {title?, description?}` (название и описание группы — админ; хотя бы одно поле, пустое тело — `400`. `description` ≤ 1000 знаков, пустая строка его убирает; системное сообщение `title_changed` создаётся только при смене названия — правка описания тихая, иначе лента засоряется. Описание есть только у пользовательских `GROUP` и приходит в списке чатов полем `description`, у остальных типов — `null`) · `DELETE /chats/:id` (владелец GROUP удаляет группу; иначе выход/удаление у себя) · `POST /chats/:id/clear` (очистка истории «для меня». Пустое тело — всё до текущего момента через `ChatMember.clearedAt`, накопленные периоды при этом сбрасываются: они и так уходят под новую границу. Тело `{from, to}` (ISO, обе границы или ни одной) — вырезать только этот период: он ложится в `ChatMember.clearedRanges` (Json-массив `[{from,to}]`, хранится слитым и отсортированным, до 50 кусков — дальше `400` с предложением очистить чат целиком). Водоразделом одной отметки времени дыру в середине истории не выразить, отсюда второе поле. Это скрытие «у себя»: чужие сообщения не удаляются, у остальных участников переписка цела. Оба фильтра применяет `clearedWhere` — один помощник на историю, догон, медиа и ссылки; поиск по сообщениям очищенное по-прежнему находит) · `GET /chats/:id/media/calendar?from=&to=&tzOffset=` (по одной миниатюре на день окна — кружки чисел в календаре перехода по дате, §5 карты интерфейса; `tzOffset` — минуты `getTimezoneOffset` смотрящего, день считается в его поясе; просматривается до 500 свежих снимков окна, presigned-ссылки подписываются пачкой) · `POST|DELETE /chats/:id/members/:userId/admin` (назначить/снять админа — только создатель) · `POST /chats/:id/transfer/:userId` (передать владение — только создатель). Отправка троттлится 20/10с на пользователя (`RATE_LIMIT`); счётчик в Redis (`chat:flood:<userId>`, INCR+EXPIRE), поэтому лимит общий для всех инстансов и самоочищается. Все пять путей отправки (WS-сообщение, multipart, опрос, пересылка, «поделиться постом») проходят одну проверку `assertCanSend`: антифлуд + личная блокировка + потолок непринятого запроса (§50). До этого пересылка и share шли мимо и обходили и блокировку, и лимит. Список чатов отдаёт `avatarUrl` (для PRIVATE — аватар собеседника), `unreadCount`, `pinned` (закреплённые сортируются сверху), `isOwner`, `isAdmin`, `blocked`/`blockedBy`; `GET /chats/:id/members` — `banned` и `isAdmin`. Список чатов отдаёт также `requestIncoming`/`requestOutgoing` (§50). Модель: `Chat.avatarUrl/createdById/lastSeq/requestPendingForId/description`, `Message.seq` (монотонный номер внутри чата, уникальный `[chatId, seq]`; аллокатор — атомарный инкремент `Chat.lastSeq`), `ChatMember.bannedAt/isAdmin/clearedAt/clearedRanges/pinnedAt/muteImportantOnly`, модели `UserBlock`, `ChatFolder`/`ChatFolderItem` (§2). Текстовые сообщения/typing/статусы/ответы (`replyToId`) — через WS (`ChatGateway`). Вложения — REST: до `FILE_UPLOAD.DIRECT_UPLOAD_THRESHOLD_BYTES` (10 МБ) multipart-запросом (`POST /chats/:id/messages`), больше — прямой загрузкой в хранилище мимо API: `POST /chats/:id/attachments/presign` (одиночный PUT) либо `POST /chats/:id/attachments/multipart/start|urls` (файлы от 50 МБ, части по 10 МБ), а затем `POST /chats/:id/messages/uploaded` с ключами объектов — там же сервер собирает многочастные и привязывает файлы к сообщению. Пара presign/multipart своя, а не общая из `/files`: членство в чате проверяется на каждом шаге. Бакет `chat-media`, приватный. Системные сообщения группы (§20): `Message.systemType` (member_added/member_removed/member_left/admin_granted/admin_revoked/title_changed/avatar_changed/message_pinned/owner_changed) + `systemMeta` {targetName?, title?}; эмитятся `message:new` при событиях состава/названия/аватара/админа/владельца/закрепления; текст рендерит клиент по типу. Опросы: `ChatPoll`/`ChatPollOption`/`ChatPollVote` (§38, WS `poll:updated`). `GET /chats/saved` (§15: личный self-chat «Сохранённые» — `ChatType.SAVED`, единственный участник; найти/создать). Ответ с цитатой фрагмента: `Message.replyQuote` — копия выделенного куска (не смещения: оригинал могут отредактировать), допустима только вместе с `replyToId`. Тихая отправка: `Message.silent` — сообщение доставляется как обычно, уведомления и push по нему не создаются. Спойлер (§34): `File.spoiler` (multipart-поле `spoiler` в `POST /chats/:id/messages` помечает все вложения, `spoilerIndexes` — номера отдельных вложений в порядке отправки, `«0,3»`; клиент размывает до клика). Два поля, а не одно: в альбоме из десяти снимков скрыть могут один, и общим флагом это не выражается; номера вне диапазона игнорируются. Отправка «без сжатия» (§9): `File.asDocument` (multipart-поле `asFiles` в том же маршруте помечает все вложения сообщения) — получатель видит строку файла с расширением и размером, а не превью, даже если mime картиночный. Флаг отправителя, а не свойство байтов: один и тот же снимок можно послать превью или документом, и показать нужно ровно выбранное. Помечаются все вложения сообщения разом, потому что способ отправки выбирается один раз на отправку. Размеры изображения: `File.width`/`File.height` заполняются при загрузке по содержимому (sharp читает заголовок) и приходят в `media` каждого сообщения — клиент резервирует под снимок точное место и держит скелетон той же формы, пока идут байты; у видео, документов и вложений старше этих полей — `null` (клиент рисует усреднённую заглушку).
...
**Жалобы** — `POST /complaints` (Student/Starosta/Teacher, 10/час) · `GET /complaints` (Moderator+, scope; фильтры `status/priority`, offset-пагинация (`limit` ≤ 200) + сортировка `?sort=priority|createdAt|status|targetType&order=asc|desc`; порядок по умолчанию — очередь: необработанные → приоритет HIGH раньше LOW → свежие раньше) · `GET /complaints/:id` · `GET /complaints/:id/messages` (доступ к чату по жалобе, пишется в аудит) · `PATCH /complaints/:id/resolve` (DELETE_CONTENT / BLOCK_USER / WARN_USER / DISMISS)

*Разбор.* `PATCH /complaints/:id/resolve` принимает `applyToDuplicates` — закрыть тем же статусом все необработанные жалобы на ту же цель в scope решающего. Побочное действие (снять контент, заблокировать) выполняется РОВНО ОДИН раз: вторая блокировка того же человека записала бы в журнал событие, которого не было. `PATCH /complaints/:id/reopen` возвращает разобранную жалобу в очередь и стирает решение, но **побочные действия не отменяет**: снятый контент не возвращается, блокировка снимается отдельно. `GET /complaints/:id` отдаёт `targetReports` — сколько всего жалоб на ту же цель; число отличает единичную обиду от травли — и `targetOwnerId`: кто отвечает за цель. В жалобе на пост или сообщение автора не видно, а блокируют человека; у снесённой цели поле приходит `null`, и карточка нарушителя просто не показывается.

*Квитирование.* `PATCH /complaints/:id/take` (Moderator+, `@MiniAllowed()`) ставит `REVIEWING` и запоминает, кто взял (`reviewing_by_id`). Уведомление о срочной жалобе уходит всей команде, и без отметки двое открывают одно и то же, а третья жалоба не достаётся никому. Перехватить чужую нельзя: условие `reviewingById: null` стоит в самом запросе, два одновременных «беру» не победят оба. Возврат в очередь (`reopen`) отметку снимает. То же действие делает кнопка под уведомлением в Telegram (см. «Вебхук бота»).

*Вебхук бота.* `POST /telegram/webhook` — **публичный** маршрут: Telegram приходит без нашего токена, и иначе быть не может. Защита одна — секрет `TELEGRAM_WEBHOOK_SECRET`, который бот присылает в заголовке `x-telegram-bot-api-secret-token` (ставится при `setWebhook`); секрет не задан — маршрут отвечает `200` и не делает ничего, потому что открытая ручка без проверки хуже её отсутствия. Ответ всегда `200`: обновление, на которое не ответили, Telegram повторяет часами и в итоге отключает вебхук. Из тела запроса берётся ТОЛЬКО `telegram_id` нажавшего; роль, scope и права читаются из `telegram_accounts` и `users` — иначе достаточно было бы прислать «я админ» вместе с нажатием. Обрабатывается одно обновление — `callback_query` с `take:ticket:<id>` или `take:complaint:<id>`; в ответ уходит всплывающая подсказка и снимается кнопка. Кнопка «Беру в работу» появляется под уведомлением только если секрет настроен: кнопка, нажатие которой некому принять, молча ничего не делает.

*Шкала решений.* `WARN_USER` — промежуточная мера между «нарушения нет» и блокировкой: до неё на первый грубый комментарий приходилось выбирать между «ничего» и отключением от платформы. Предупреждение не трогает ни контент, ни доступ; человеку уходит уведомление о самом факте, а внутренний комментарий модератора ему **не пересылается** — его писали не нарушителю. Запись ложится в `moderation_warnings`, а не в `AuditLog`: журнал чистится ежемесячно (`cleanAuditLogs`), а «второе предупреждение за месяц» и «первое за два года» — разные случаи, и различает их только сохранённая история. Кода 2FA предупреждение не требует.

*Срок блокировки.* `BLOCK_USER` принимает `blockDays` (1…365) — `PATCH /users/:id/block` тоже. Срок пишется в `users.blocked_until` и считается от момента решения: «на семь дней», выданное вечером, кончается вечером. Без срока блокировка бессрочная, как была. Снимает её крон `liftExpiredBlocks` (каждые 5 минут) — тем же способом, каким снимаются техработы; разблокировка руками срок стирает, иначе одна и та же блокировка «снималась» бы дважды. Потолок в год намеренный: всё, что дольше, по смыслу и есть «навсегда», а промах в поле не должен создавать блокировку до 2147 года.

*Жалоба из обращения.* `POST /complaints/from-support` (`PLATFORM_ADMIN`/`PLATFORM_MODERATOR`, `@MiniAllowed()`; тело `{ chatId, targetId }`) заводит жалобу на человека по обращению в поддержку. Люди жалуются на других людей через поддержку — адрес известен, а кнопку «пожаловаться» рядом с обидчиком ещё надо найти; до этой ручки путь кончался тупиком. **Автором жалобы остаётся автор обращения**, а не модератор: жаловался он, и очередь должна показывать именно это — иначе по статистике половину жалоб на платформе подаёт поддержка. Текст берётся из первого сообщения обращения (его собственные слова, до 2000 символов). Тип цели не спрашивается: через поддержку жалуются на людей, а `targetId` поста в переписке всё равно нет.

*История по цели.* `GET /complaints?targetId=` — все жалобы на один объект или человека. Единичная обида и травля в очереди выглядят одинаково; разводит их только список прошлых разборов.

*Накопление очереди.* Ежечасный cron `alertQueueBacklog` (`CleanupService`) сигналит в Telegram, когда необработанных жалоб становится больше 20, и молчит следующие 6 часов: разгрести два десятка за час нельзя, а пять одинаковых сообщений подряд учат их не читать. Как только очередь опускается ниже порога, пауза снимается — следующий всплеск не пропустится.
...
*Область данных карьерного центра.* Разделы вуза (допуск компаний, модерация вакансий, события, метрики) работают в scope ОДНОГО вуза. У сотрудника он берётся из токена; переданный в запросе чужой `universityId` → 403 `WRONG_SCOPE`. Платформенные роли своего вуза не имеют и указывают его параметром `?universityId` (в решениях — полем тела): без него — 403 `WRONG_SCOPE` «Выберите университет». Прав параметр не добавляет: платформенные роли и так видят любой вуз, он лишь выбирает, какой показать. Единая реализация — `career-scope.ts` (`resolveUniversityScope`), на фронте вуз живёт в query-параметре страницы и выбирается селектором в шапке раздела.
...
*Допуск к вузу.* `GET /career/university/companies` + `PATCH /career/university/companies/:id` (решение — `PLATFORM_ADMIN`, `UNIVERSITY_ADMIN`; чтение — ещё `PLATFORM_MODERATOR`/`UNIVERSITY_MODERATOR`/`DEAN`) — очередь заявок компаний и решение по ним. Статусы `CompanyUniversityAccess.status` = `REQUESTED|APPROVED|REJECTED|REVOKED`, у одобренного возможен `expiresAt`. **Отзыв (`REVOKED`) каскадом закрывает вакансии этой компании в этом вузе**: тем же транзакционным шагом все `VacancyUniversityReview` компании по этому вузу (и `APPROVED`, и `PENDING`) переводятся в `REJECTED` с причиной «Университет отозвал допуск компании». Вузы, где допуск остался, не затрагиваются. Отдельного статуса `REVOKED` у решения по вакансии нет намеренно — вернут допуск, компания публикует заново, вуз рассматривает с нуля.
...
*Вакансии.* Студенту и сотрудникам вуза (`TEACHER`, `DEAN`, `UNIVERSITY_ADMIN`, `UNIVERSITY_MODERATOR`) — `GET /career/vacancies`, `GET /career/vacancies/:id` (показываются только одобренные их вузом; платформенным ролям — витрина выбранного вуза по `?universityId`). Работодателю — `GET|POST /career/employer/vacancies`, `PATCH /career/employer/vacancies/:id` и переходы `POST :id/publish|pause|close` (`Vacancy.status` = `DRAFT|PUBLISHED|PAUSED|CLOSED`). Вузу — `GET /career/university/vacancies` + `PATCH :id` (модерация; чтение — ещё `DEAN`, решение — `UNIVERSITY_ADMIN`/`UNIVERSITY_MODERATOR` и обе платформенные роли; `VacancyUniversityReview.status` = `PENDING|APPROVED|REJECTED`): вакансия видна в вузе только после одобрения **этим** вузом.
...
*Отклики.* Студент: `GET|POST /career/applications`, `GET :id/history`, `POST :id/withdraw`. Работодатель: `GET /career/employer/applications`, `PATCH :id` (смена статуса). Воронка `CareerApplication.status` = `SUBMITTED|VIEWED|SHORTLISTED|INTERVIEW|OFFER|HIRED|REJECTED|WITHDRAWN`, каждый переход пишется в `CareerApplicationEvent`. Отзыв допуска вузом на уже поданные отклики не влияет: выборка откликов студента намеренно не фильтруется по решениям вуза, и в карточке остаются название компании, её `website` и полная история статусов — связь с работодателем вне платформы должна пережить решение вуза, иначе диалог обрывается на середине.

*Прочее.* `GET /career/events` — карьерные мероприятия (все роли вуза и обе платформенные) (`Event.careerKind` = `CAREER_FAIR|WORKSHOP|INTERVIEW_DAY|COMPANY_PRESENTATION|HACKATHON`). `GET /career/analytics/university` (роли вуза, кроме `TEACHER`, и обе платформенные) и `GET /career/analytics/company` (`EMPLOYER`) — воронка и срезы.

Сводка вуза (`GET /career/analytics/university`, кэш Redis 60 с, ключ `analytics:career:university:<id>`): `companies`/`vacancies` — решения по статусам; `funnel` — где отклики лежат **сейчас** (включая `REJECTED`/`WITHDRAWN`); `reached` — сколько откликов **дошло** до шага (`SUBMITTED|VIEWED|SHORTLISTED|INTERVIEW|OFFER|HIRED`, накопительно, по максимуму из текущего статуса и `CareerApplicationEvent`) — только на нём строятся воронка и `rates`, потому что группировка по текущему статусу теряет и найм, и отказ после интервью; `queue` — очередь и сроки (вакансий на модерации, возраст старейшей, медиана решения за 90 дней, заявки компаний, допуски с истечением в 30 дней, отклики без ответа дольше 14 дней); `weekly` — 12 недель (`submitted`/`interview`/`hired`); `skills` — топ-8 навыков «спрос вакансий, одобренных этим вузом, против предложения студентов». Ряды считаются в SQL (`date_trunc`/`generate_series`/`FILTER`/`unnest`), персональных данных нет.

Отчёт за период (`GET /career/analytics/university/report?period=month|quarter|year`, кэш Redis 5 мин, ключ `analytics:career:report:<id>:<period>`; `year` — **учебный** год с 1 сентября): `totals` — объём за период и за предыдущий той же длины (дельту считает экран); `timing` — медиана до первого ответа компании (`SUBMITTED → VIEWED`), медиана до найма и доля откликов без реакции; `funnel` — шаги за период и потери между соседними; `outcomes` — найм/отказ/отзыв/в работе; `faculties` — трудоустройство по факультетам **с порогом малых групп** (факультет меньше 5 студентов не показывается вовсе, число скрытых возвращается в `suppressed`); `graduates` — выпускной курс текущего учебного года; `companies` — топ-10 по найму; `vacancyCuts` и `salary` — снимок витрины на сегодня (занятость/формат/опыт/города, медианы вилок и ожиданий); `deadVacancies` — одобренные вакансии без единого отклика; `students` — корзины `readinessScore`, статусы поиска, опубликованные резюме, открытые профили. Ряда по мероприятиям в отчёте намеренно нет: `EventParticipant` фиксирует только запись, явки в модели не существует, а регистрации под видом посещаемости врали бы в отчёте.
...
| `typing:start` / `typing:stop` | `{ chatId }` — то же самое для клиентов прошлой версии: сервер разворачивает их в `chat:action` с `TYPING` и `null` |
...
| `chat:action` | `{ chatId, userId, action }` — в личные комнаты участников (`user:{id}`), чтобы подпись была видна и в строке списка чатов, а не только с открытым чатом. Веер ограничен 64 участниками (`TYPING_FANOUT_MAX`): в чате на сотни человек кто-то печатает всегда, строка перестала бы что-либо значить, а отправок было бы сотни каждые три секунды. Сверх потолка — в комнату `chat:{id}`. Членство отправителя проверяется тем же запросом, которым берутся получатели: адресация идёт мимо комнаты, и «я в неё вошёл» не защита |
| `typing:started` · `typing:stopped` | `{ chatId, userId }` — дубль `chat:action` для клиентов прошлой версии, только для `TYPING` и конца действия. Остальные действия старому клиенту не шлются вовсе: перевести он их не сумеет |
| `chat:updated` | `{ chat }` |
...
### 9.1a Действия в чате

Подпись «печатает…» обобщена до действия: `TYPING`, `RECORDING_VOICE`, `UPLOADING_PHOTO`,
`UPLOADING_VIDEO`, `UPLOADING_FILE` (контракт — `ChatActionSchema` в `@studenthub/shared-schemas`).
Это **не тип сообщения**: типа сообщения в модели нет вовсе, вид вложения выводится при рендере
из mime, имени файла (`voice-…`) и флага `asDocument`. Действие живёт секунды, в БД и в Redis
не попадает — только событие и память вкладки.

Время жизни держит получатель: подпись гаснет через 4 с без подтверждения, отправитель повторяет
его раз в 3 с. Поэтому «стоп» не обязателен — закрытая вкладка, разрыв связи и упавший сервер
гасят подпись сами, а `startedAt`/`expiresAt` в событии не нужны.

Набор текста подтверждается нажатиями и явного «стоп» не шлёт (кроме отправки сообщения и ухода
из чата). Запись голосового и загрузка вложения длятся сами, поэтому подтверждаются по таймеру и
всегда снимаются в `finally`. Загрузка сообщается не сразу, а через 400 мс: сжатый снимок
улетает за доли секунды, и подпись успела бы только мигнуть.

Текст собирает интерфейс: сервер шлёт только значение действия, i18n-ключи — `Chats.actions.*`
(мн. ч. по числу участников), `Chats.actionsNamed.*` (с именем, в групповых чатах) и
`Chats.actionsMany` (когда участники делают разное — перечислять их в одну строку негде).
...
| `rotateDuty` | `0 9 * * 1` | Дежурство переходит к следующему в очереди |
| `liftExpiredBlocks` | `*/5 * * * *` | Временные блокировки с истёкшим `blocked_until` → снятие + запись `user_unblocked_expired` |
...
# Брендирование выгружаемых документов (§8.3 «Выгрузки и проверка документов»).
# APP_PUBLIC_URL уходит ВНУТРЬ выданных файлов (колонтитул, QR, ссылка проверки), поэтому
# отдельно от CORS_ORIGIN: смена настроек CORS не должна менять адрес в уже выданных бумагах.
# Пусто — берётся первый origin из CORS_ORIGIN.
...
### 14.1 Второй вариант: тестовый стенд (`pnpm db:seed:test`)

Отдельная точка входа `prisma/seed-test.mjs` рядом с основным сидом. Состав задан
поимённо, а не выведен из числа студентов, — стенд заводят под приёмку, и числа в нём
проверяются:
...
| Университет | **1** (демо-вуз «Алатау» здесь не заливается) |
| `UNIVERSITY_ADMIN` | 1 |
| `UNIVERSITY_MODERATOR` | 2 |
| `DEAN` | 2 (по одному на факультет) |
| `TEACHER` | 24 (12 + 12) |
| `STUDENT` + `STAROSTA` | **1450** = 1392 + 58 (староста — по одному на группу) |
| `EMPLOYER` | 27 (20 компаний, иначе раздел «Карьера» пуст) |

Структура: 2 факультета × 29 групп по 25 учащихся = 58 групп. Заполнено всё остальное —
справочники (КАТО, каталог услуг, типы документов), расписание, оценки, посещаемость,
экзамены, документы, заявки, чаты, посты, статьи, опросы с голосами, портфолио, вакансии
и отклики. Профили пользователей заполнены целиком: пустое поле означает не отрисованный
блок на экране, а стенд заводят как раз чтобы эти блоки посмотреть.

**Почему отдельная точка входа, а не флаг.** Требование «1 университет» означает ровно
один, а основной сид безусловно заливает демо-вуз с named-аккаунтами: он вшит в его 1300
...
**Личная галерея — пустая.** Пул медиа (200 фото, 50 видео) заливается: из него идут
аватары, обложки профилей, обложки альбомов и статей, картинки постов. А вкладки «Фото»
и «Видео» в профиле остаются пустыми — по решению от 2026-09-16.

Причина в цене. `File` уникален по `(bucket, key)`: карточка галереи — это отдельный
объект в MinIO, а не ссылка на пул, поэтому «сто фото у каждого» означает 150 тыс.
объектов. Считается так: 100 фото × 1509 пользователей ≈ 13,5 ГиБ, плюс видео (тяжелее
фото примерно в 38 раз) — итого ≈ **29 ГиБ, то есть 29 из 30 ГБ всего стенда** ради
содержимого двух вкладок.

Механизм (`prisma/seed/steps/57-user-media.mjs`) не удалён, а выключен нулём и включается
без пересборки остального — копии делает сам MinIO (`copyObject`), заново ничего не
качается:

```bash
SEED_PHOTOS_PER_USER=30 SEED_VIDEOS_PER_USER=1 pnpm db:seed:test   # ~9 ГБ, +~1 мин
```

Шаг вызывается только из `seed-test.mjs`; в `demo`/`small`/`full` его нет вовсе.

Состав переопределяется теми же ручками: `SEED_FACULTIES`, `SEED_TEACHERS`,
`SEED_GROUP_SIZE`, `SEED_STUDENTS_MIN`/`MAX`, `SEED_PLATFORM_MODERATORS`. Стенд
пересобирается на пустой БД; прогон без медиа — около минуты, с галереей — десятки минут.
