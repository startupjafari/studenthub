...
**Все сервисы из репозитория обязаны собираться с ОДНОЙ ветки** (Settings → Source →
Branch). Ветка задаётся в интерфейсе Railway, а не в `railway.json`, поэтому разъезд
случается тихо: сервис, оставленный на `develop`, начинает запрашивать у сервиса на `main`
маршруты, которых там ещё нет. Симптом — 404 на новых ручках при внешне рабочем
приложении. Проверять при заведении каждого нового сервиса.
...
2. **Variables** — задать:
   ...
   `SENTRY_RELEASE` стоит задать сразу как `${{RAILWAY_GIT_COMMIT_SHA}}`: без него
   ошибка в Sentry и сообщение ops-бота о деплое приходят с версией `dev`, и «когда
   это сломалось» приходится выяснять по времени. Порядок выпуска — `docs/RELEASE.md`.
...
3. **Variables**:
   ...
   (тот же `${{RAILWAY_GIT_COMMIT_SHA}}`, что и у api — версия сборки уже показывается
   пользователю в настройках, `NEXT_PUBLIC_BUILD_ID`)
...
Админский мини-апп в Telegram (`docs/PROJECT.md §Мини-апп`). Статика за nginx, который
здесь же проксирует `/api` на бэкенд.

1. **+ New → GitHub Repo** → тот же репозиторий (четвёртый сервис из репо).
2. **Config-as-code path** = `apps/mini-app/railway.json`.
3. **Variables**:

   | Переменная | Значение |
   |---|---|
   | `API_TARGET` | `https://api.<домен>` — адрес сервиса `api` |

   > `PORT` Railway подставляет сам. Переменная читается **при старте** контейнера
   > (envsubst по шаблону nginx), поэтому её правка требует рестарта, а не пересборки.

4. **Generate Domain** — этот адрес и прописывается в `@BotFather`: `/setmenubutton` →
   бот → адрес → текст кнопки.

Почему прокси, а не прямые запросы с фронта на `https://api.<домен>`: мини-апп и API
оказываются одним origin'ом, поэтому `CORS_ORIGIN` сервиса `api` трогать не нужно, а адрес
бэкенда не вшивается в сборку. В разработке ровно так же работает прокси Vite — поведение
не расходится между dev и продом.

**Ветка сборки — та же, что у `api` и `web`.** Мини-апп ходит в бэкенд за жалобами,
обращениями и состоянием платформы; собранный с более свежей ветки, он запрашивает
маршруты, которых на бэкенде ещё нет, и все экраны показывают ошибку загрузки.

**Что нужно на стороне `api`**, иначе мини-апп покажет экран привязки и не примет код:

- применённая миграция `telegram_accounts` (§5);
- переменная `TELEGRAM_BOT_TOKEN` — токен того же бота, в меню которого стоит кнопка;
- переменная `MINI_APP_URL` — адрес этого сервиса; без неё уведомления в Telegram уходят
  без кнопки «Открыть».
  Без неё `/mini/session` отвечает «нет доступа» и пишет причину в лог.
- переменная `TELEGRAM_WEBHOOK_SECRET` (16+ символов) и вызов `setWebhook` у бота с тем же
  `secret_token` и адресом `https://<api>/api/v1/telegram/webhook` — только после этого под
  уведомлениями появляется кнопка «Беру в работу». Без переменной вебхук отвечает 200 и
  ничего не делает, а кнопка не рисуется: нажатие, которое некому принять, хуже её отсутствия.
...
### CORS бакетов: обязательно для крупных файлов

Файлы больше 10 МБ браузер кладёт в хранилище сам, по подписанной ссылке, минуя `api` — а
больше 50 МБ ещё и частями (Фаза 19). Оба пути — кросс-доменные запросы к MinIO, поэтому
бакетам нужна CORS-политика:

```json
{
  "CORSRules": [
    {
      "AllowedOrigin": ["https://app.<домен>"],
...
Применяется на каждый бакет: `mc admin` или `mc cors set <alias>/<bucket> cors.json`.

**`ExposeHeader: ETag` — не деталь, а условие работоспособности.** Сборка многочастной
загрузки требует `ETag` каждой части, и браузер их не увидит, если заголовок не открыт явно.
Симптом при этом обманчивый: части уходят до конца, полоса доходит до ста процентов, и только
финальный шаг падает — выглядит как случайный сбой на большом файле, а не как забытая
настройка. Клиент в этом случае отдаёт ошибку, прямо называющую `ExposeHeaders`.

### Том под крупные файлы

Предел видео и документов — 500 МБ (`FILE_UPLOAD.MAX_BYTES`), и это прямо переводится в размер тома
на `/data`. Незавершённые части тоже занимают место: их отменяет ночная задача
`sweepIncompleteUploads` (старше суток), но лучше продублировать правилом на бакете —
`AbortIncompleteMultipartUpload` работает и тогда, когда приложение лежит.

Трафик загрузки идёт в minio-сервис напрямую, **мимо `api`**. Лимиты тела запроса и таймауты
маршрутов `api` к нему отношения не имеют — упереться можно только в том и в сеть.
...
**Порядок: миграция → выкатка кода, а не наоборот.** Новый код на старой схеме падает на
первом же запросе к недостающей колонке, и падение не обязательно ограничивается новой
функцией: код, который читает что-то на каждом запросе, уронит весь API. Обратный порядок
(схема впереди кода) безопасен: миграции аддитивные, а старый код о новых колонках
не знает.
