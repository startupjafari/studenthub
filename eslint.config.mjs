...
export default tseslint.config(
  {
    ignores: [
      ...
      '**/dist-e2e/**',
      '**/build/**',
      // Любой каталог сборки Next: обычный `.next`, стенд e2e (`.next-e2e`) и `.next-build`,
      // в который AGENTS.md предписывает собирать, чтобы не затирать `.next` у живого dev.
      // Без звёздочки после `.next` линт заходил в `.next-build` и выдавал полторы сотни
      // ошибок в минифицированном выводе сборки.
      '**/.next*/**',
      '**/out/**',
      '**/.turbo/**',
      '**/coverage/**',
