...
export class AdminController {
  ...
  @ApiOperation({
    ...
    description:
      'Searches users in the same university for role assignment. Only accessible to university admins.',
  })
  ...
  async searchUsers(@Query() dto: AdminSearchUsersDto, @CurrentUser() user: User) {
  ...
  @ApiOperation({
    ...
    description:
      'Assigns TEACHER role to a user. Creates Teacher record, updates User role, and removes Student record if exists.',
  })
  @ApiParam({ name: 'id', description: 'User ID', type: String })
  @ApiResponse({ status: 201, description: 'Teacher role assigned successfully' })
  @ApiResponse({
    status: 400,
    description: 'Bad request - User is already a teacher or employee ID exists',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only university admins can assign teachers',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Removes TEACHER role from a user. Changes role back to STUDENT, deletes Teacher record, and creates Student record.',
  })
  @ApiParam({ name: 'id', description: 'Teacher ID', type: String })
  @ApiResponse({ status: 200, description: 'Teacher role removed successfully' })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only university admins can remove teachers',
  })
  @ApiResponse({ status: 404, description: 'Teacher not found' })
  async removeTeacher(@Param('id') teacherId: string, @CurrentUser() user: User) {
  ...
  @ApiOperation({
    ...
    description:
      'Returns paginated list of approved teachers in the university. Supports filtering by department and search.',
  })
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Only university admins can access this' })
  async getTeachers(@Query() dto: SearchTeachersDto, @CurrentUser() user: User) {
  ...
  @ApiOperation({
    ...
    description:
      'Returns paginated list of students in the university. Supports filtering by academic status and search.',
  })
  @ApiQuery({ name: 'search', required: false, description: 'Search query' })
  @ApiQuery({
    name: 'academicStatus',
    required: false,
    enum: ['ACTIVE', 'INACTIVE', 'GRADUATED', 'EXPELLED', 'ON_LEAVE'],
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Only university admins can access this' })
  async getStudents(@Query() dto: SearchStudentsDto, @CurrentUser() user: User) {
  ...
  @ApiOperation({
    ...
    description:
      'Returns statistics about the university: total students, teachers, groups, active students, and graduated students.',
  })
  @ApiResponse({
    status: 200,
