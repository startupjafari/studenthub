...
export class NotificationsController {
  ...
  @ApiOperation({
    ...
    description: 'Returns paginated list of user notifications. Supports filtering by read status and type.',
  })
  @ApiQuery({ name: 'isRead', required: false, type: Boolean, description: 'Filter by read status' })
  @ApiQuery({ name: 'type', required: false, enum: ['FRIEND_REQUEST', 'FRIEND_ACCEPTED', 'MESSAGE', 'POST_COMMENT', 'POST_REACTION', 'STORY_VIEW', 'GROUP_INVITE', 'EVENT', 'TEACHER_MESSAGE', 'DOCUMENT_REQUEST', 'DOCUMENT_UPDATED', 'ROLE_CHANGED'], description: 'Filter by notification type' })
  ...
  @ApiResponse({ status: 200, description: 'Unread count retrieved successfully', schema: { type: 'object', properties: { count: { type: 'number' } } } })
  ...
  @ApiOperation({
    ...
    description: 'Deletes a notification. Only the notification recipient can delete their own notifications.',
  })
  @ApiParam({ name: 'id', description: 'Notification ID', type: String })
