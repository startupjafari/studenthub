...
export class UniversitiesController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - University with this name already exists',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only super admins can create universities',
  })
  async createUniversity(@Body() dto: CreateUniversityDto, @CurrentUser() user: User) {
  ...
  @ApiOperation({
    ...
    description:
      'Updates university details (name, country, city, address). Only super admins can update universities.',
  })
  @ApiParam({ name: 'id', description: 'University ID', type: String })
  @ApiResponse({ status: 200, description: 'University updated successfully' })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only super admins can update universities',
  })
  @ApiResponse({ status: 404, description: 'University not found' })
  ...
  @ApiOperation({
    ...
    description:
      'Assigns UNIVERSITY_ADMIN role to a user for a specific university. Creates UniversityAdmin record and updates User role.',
  })
  @ApiParam({ name: 'id', description: 'University ID', type: String })
  @ApiResponse({ status: 201, description: 'Admin assigned successfully' })
  @ApiResponse({
    status: 400,
    description: 'Bad request - User is already an admin of this university',
  })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only super admins can assign university admins',
  })
